      ...
      <div className="sm:px-6 pt-4 sm:pt-6 pb-10">
        {/* HEADER */}
          ...
          <p className="text-xs sm:text-[13px] text-[#64748B] mt-1">
            {sortedEvents.length} upcoming BRICS events
          </p>
        </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {sortedEvents.length === 0 && (
                  <tr>
                    <td colSpan="7" className="text-center py-6 text-base">
                ...
                {sortedEvents.map((e) => (
                  <tr key={e.id} className="border-b">
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="min-w-0">
                          <p className="text-base capitalize text-[var(--color-primary-blue)] font-semibold truncate max-w-[300px]" title={e.name || ''}>{e.name}</p>
                          <p className="text-sm capitalize text-[var(--color-primary-blue)] truncate max-w-[300px]" title={`${e.location || ''}${e?.venue ? `, ${e.venue}` : ''}`}>{`${e.location || ''}${e?.venue ? `, ${e.venue}` : ''}`}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <span className="block truncate max-w-[220px] text-[var(--color-primary-blue)]" title={`${e.location || ''}${e?.venue ? `, ${e.venue}` : ''}`}>{`${e.location || ''}${e?.venue ? `, ${e.venue}` : ''}` || '-'}</span>
                    </td>
                    ...
                    <td className="px-4 py-4 text-[var(--color-primary-blue)] text-base">{e.delegate_count ?? e.delegateCount ?? 0}</td>
